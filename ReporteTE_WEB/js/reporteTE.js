...
function produccion(cont) {
	...
	if (parseInt(cont) == parseInt($("#cantActualizaciones").text())) {
			$.ajax({
				url: baseURL+'reporteTE/estructurarEncabezado',
				type: 'POST',
				dataType: 'json'
			}).done(function(informacion) {
				// ...
				$.ajax({
					url: baseURL+'reporteTE/consultarInformacionCuerpoTabla',
					type: 'POST',
					dataType: 'html',
...
function estadoDelServidorDB() {
		// ...
			...
			$.ajax({
				url: baseURL+'reporteTE/estadoDeConexionServidor',
				type: 'POST'
			})
...
function estadoLectura() {
	$.ajax({
		url: baseURL + 'reporteTE/estadoDeLecturaPuertoSerial',
		type: 'POST'
	})
